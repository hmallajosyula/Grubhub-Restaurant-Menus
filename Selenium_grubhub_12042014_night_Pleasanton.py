...
sys.stdout = Logger(strftime("Logfiles/Pleasanton_Restaurant_Menus_%H_%M_%m_%d_%Y.txt"))
...
br.get('https://www.grubhub.com/search?orderMethod=pickup&locationMode=PICKUP&facetSet=umami&pageSize=20&latitude=37.66251373&longitude=-121.87461854&radius=5&countOmittingTimes&pageNum=1')
time.sleep(30)
...
for j in range(1,2):
        try:
            time.sleep(15)
            search = br.find_element_by_xpath('//*[@id="ghs-search-results-container"]/div[2]/div/div/div/div[2]/div/div/div[1]/ul/li[7]')
            search.click()
        except ElementNotVisibleException:
                br.implicitly_wait(15)
                seconds = 15 + (random.random()*5)
                time.sleep(seconds)
                break
        except NoSuchElementException:
