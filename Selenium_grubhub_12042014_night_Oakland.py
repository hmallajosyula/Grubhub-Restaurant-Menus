...
sys.stdout = Logger(strftime("Logfiles/Oakland_Restaurant_Menus_%H_%M_%m_%d_%Y.txt"))
...
br.get('https://www.grubhub.com/search?orderMethod=pickup&locationMode=PICKUP&facetSet=umami&pageSize=20&latitude=37.804363&longitude=-122.27111&radius=5&countOmittingTimes')
time.sleep(30)
...
for j in range(1,8):
        try:
            time.sleep(15)
            search = br.find_element_by_xpath('//*[@id="ghs-search-results-container"]/div[2]/div/div/div/div[2]/div/div/div[1]/ul/li[8]')
            search.click()
        except ElementNotVisibleException:
                br.implicitly_wait(15)
